...
def convert_abundance(tsv_file, output_prefix):
    ensembl_root = os.getenv('ENSEMBL_ROOT')
    # Load the tx2gene file (transcript to gene mapping)
    if ensembl_root is None:
        print("Error: ENSEMBL_ROOT environment variable is not set.")
    else:
    # Construct the full path to the tx2gene file
        tx2gene_file = os.path.expandvars("$ENSEMBL_ROOT/Homo_sapiens.GRCh38.Ensembl104.tx2gene")
    
    # Read the file
    tx2gene = pd.read_csv(tx2gene_file, sep="\t")
    print(tx2gene.head())  # Print the first few rows for inspection
    ...
    abundance_data = abundance_data.merge(tx2gene, how='left', left_on='target_id', right_on='tx_id')
    print(abundance_data[abundance_data['gene_id'] == 'ENSG00000005381'])
