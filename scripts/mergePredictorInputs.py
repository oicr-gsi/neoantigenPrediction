...
def merge_predictor_inputs(variants_peptides, variant_deciles, expression_deciles, rnaseq_variants, output_prefix):
    ...
    m.to_csv(f'{output_prefix}.output.merged.tsv', sep='\t', index=False, quoting=csv.QUOTE_NONE)
    ...
    with pd.ExcelWriter(f'{output_prefix}.output.merged.xls', engine='xlsxwriter') as writer:
