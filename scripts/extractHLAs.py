...
def extract_hlas(hlafiles, hlacallers, outputFilePrefix):   
    
    if len(hlafiles) != len(hlacallers):
        print(f"Error: hlafiles and hlacallers have mismatched lengths.")
        return
        
    hlas = []
    for file in hlafiles:
        if not hlacallers:
            print(f"Error: hlacallers is empty when processing file: {file}")
            break  # Stop if hlacallers is empty
        caller = hlacallers.pop(0)
        print(f"Processing file: {file} with caller: {caller}")
    ...
    hlastring = " ".join(hlas)
    # Debug log
    print(f"Writing to file: {outputFilePrefix}.hlastring.txt")
    print(f"Total alleles: {len(hlas)}")  

    try:
        with open(outputFilePrefix, "w") as hlaout:
            hlaout.write(hlastring)
        print(f"File {outputFilePrefix}.hlastring.txt has been written")
...
if __name__ == "__main__":
    ...
    extract_hlas(hlafiles, hlacallers, outputFilePrefix)
